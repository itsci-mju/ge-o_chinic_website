...
<head>
<meta charset="UTF-8">
<title>View Appionment Page</title>
<link rel="stylesheet"
							...
							<td>
								<c:choose>
								  <c:when test="${BuyServices.status eq '0'}">
									<a href="open_print_invoice_page?id=${BuyServices.buyservice_id}"><button type="submit" class="btn btn-primary"> ชำระเงิน </button></a>
								  </c:when>
								  <c:when test="${BuyServices.status eq '1'}">
								    <a href="alert_list_rigister_page?email=<%= m.getLogins().getEmail() %>"><button type="button" class="btn btn-secondary" style="color: white;">รอการตรวจสอบ</button></a>
								  </c:when>
								  <c:otherwise>
								  	<a href="open_receipt_invoice_page?id=${BuyServices.buyservice_id}"><button type="button" class="btn btn-success"> ชำระเงินเรียบร้อย </button></a>
								  </c:otherwise>
								</c:choose> 
							</td>
							<td>
								<a href="open_make_appoinment_page?id=${BuyServices.buyservice_id}&email=<%= m.getLogins().getEmail() %>"><button type="submit" class="btn btn-warning"> <ion-icon name="create"></ion-icon></button></a>
							</td>
							<td><a href="open_cancel_appointment_page?id=${BuyServices.buyservice_id}&email=<%= m.getLogins().getEmail() %>""><button type="submit" class="btn btn-danger"> <ion-icon name="create"></ion-icon></button></a></td>
						</tr>
						</c:forEach>
					</tbody>
		</table>
